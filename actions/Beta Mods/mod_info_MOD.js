...
mod: function(DBM) {
	//Check for needed Node Modules
	DBM.Audio.ytdldiscord = null;
	try {
		DBM.Audio.ytdldiscord = DBM.Actions.getWrexMods().require('ytdl-core-discord');
	} catch(e) {};

	//Check for Data Objects & create them if needed
	...
	DBM.Audio.playItem = function(item, id) {
		if(!this.connections[id]) return false;
		...
		switch(type) {
			...
			case 'url'://Play URL
				...
				this.playingnow[id] = item;
				break;
			case 'yt'://Play YouTube Video or Livestream
				setupDispatcher = this.playYt(item[2], item[1], id);
				this.playingnow[id] = item;
				break;
		};
		if(!DBM.Audio.dispatchers[id]) return false;//If the item is a YouTube Livestream, the dispatcher is placed in this.playYt
		if(setupDispatcher && !DBM.Audio.dispatchers[id]._eventSetup) {
	...
	DBM.Audio.playYt = function(url, config, id) {
		var options = {
			seek: config.seek,
		...
		if(config.live === true) {
			//Livestream
			const ytdldiscord = DBM.Audio.ytdldiscord;
			if(!ytdldiscord) {
				console.log('ERROR: ytdl-core-discord is not installed!\nPlease make sure you run your bot via CMD.');
				return false;
			};
			async function play(url, id, DBM) {
				var dispatcher = DBM.Audio.connections[id].playOpusStream(await DBM.Audio.ytdldiscord(`${url}`, {volume: 0.5}));
				dispatcher.on('end', function() {//Because of the needed async function, the dispatcher is placed in here... You'll be unable to make any changes to it.
				const isForced = dispatcher._forceEnd;
				dispatcher = null;
				if(!isForced) {
					DBM.Audio.playNext(id);
				}
				}.bind(DBM.Audio));
				dispatcher._eventSetup = true;
			};
			play(url, id, DBM);
			return true;
		} else {
			//Video
			const ytdl = DBM.Audio.ytdl;
